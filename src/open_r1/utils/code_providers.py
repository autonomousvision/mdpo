...
class BatchResult:
    def __init__(self, results: List[Optional[float]], error: str = None):
        self.results = results
        self.error = error


class LocalSingularitySandbox:
    def __init__(self, image_path="docker://python:3.10", timeout=30):
        self.image_path = image_path
        self.timeout = timeout

    @classmethod
    async def create(cls, timeout=30, request_timeout=30):
        return cls(timeout=timeout)

    async def run_batch_scripts(self, scripts: List[str]) -> BatchResult:
        count = len(scripts)
        if count == 0:
            return BatchResult([])

        # --- The Internal Driver Script (Runs INSIDE Container) ---
        driver_code = f"""
import subprocess
import json
import sys
...
def main():
    ...
    for i in range(num_scripts):
        script_file = f"/mnt/script_{{i}}.py"

        try:
            # Execute the user script as a subprocess
            proc = subprocess.run(
                ["python3", script_file],
                capture_output=True,
                text=True,
...
"""
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            # 1. Write the Driver Script
            driver_path = os.path.join(tmp_dir, "batch_driver.py")
            with open(driver_path, 'w') as f:
                f.write(driver_code)

            # 2. Write all input scripts to files
            for i, script_content in enumerate(scripts):
                script_path = os.path.join(tmp_dir, f"script_{i}.py")

                # --- FIX IS HERE ---
                # Remove common leading whitespace and extra newlines
                clean_content = textwrap.dedent(script_content).strip()
            ...
            cmd = [
                ...
                "--bind", f"{tmp_dir}:/mnt",
                self.image_path,
                "python3", "/mnt/batch_driver.py"
            ]

            try:
                # 4. Run the Container (Just ONCE)
                ...
                if process.returncode != 0:
                    return BatchResult([], error=stderr.decode())

                output_str = stdout.decode().strip()
                try:
                    scores = json.loads(output_str)
                    return BatchResult(scores)
                except json.JSONDecodeError:
                    return BatchResult([], error=f"JSON Parse Error: {output_str}")

            except asyncio.TimeoutError:
                return BatchResult([], error="Batch Timeout")
...
class LocalExecutionProvider:
    def __init__(self, num_parallel: int = 1, image_path: str = "docker://python:3.10", **kwargs):
        self.image_path = image_path
    ...
    def execute_scripts(self, scripts: List[str], languages: List[str]) -> List[float]:
        ...
        async def _run_batch():
            sandbox = await LocalSingularitySandbox.create()
            sandbox.image_path = self.image_path
            return await sandbox.run_batch_scripts(scripts)

        result = loop.run_until_complete(_run_batch())

        if result.error or not result.results:
            # if result.error:
            #     print(f"Batch Execution Error: {result.error}")
