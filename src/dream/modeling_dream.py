...
class DreamSdpaAttention(DreamAttention):
    ...
    def forward(
        ...
        attn_output = torch.nn.functional.scaled_dot_product_attention(
            ...
            key_states,
            value_states,
            attn_mask=_prepare_4d_attention_mask_for_sdpa(attention_mask, dtype=query_states.dtype) if attention_mask is not None else None,
...
class DreamBaseModel(DreamPreTrainedModel):
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            #TODO: maybe not correct to always recompute position ids
            position_ids = torch.where(attention_mask == 1, cumsum - 1, 0)
